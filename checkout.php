...
if ($result->num_rows === 1) {
    ...
    if ($stmt->execute()) {
        header("Location: index.php?message=Success! Gift Card Purchased.&type=success");
    } else {
        header("Location: index.php?message=Error: Purchase failed.&type=error");
    }
} else {
    header("Location: index.php?message=Error: Email not found in database.&type=error");
}
